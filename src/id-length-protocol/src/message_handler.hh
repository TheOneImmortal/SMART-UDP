...
// template <u64 id, class... Ts> class call_helper<id, std::tuple<Ts...>> {
// public:
//   template <u64... Is>
...
//           tuple_front_i_size<Is, std::tuple<Ts...>>::value, sizeof(Ts)));
//     }()...);
//   };
...
template <class Handler> BufferOnHeap serialize(auto &&...var);
...
namespace helper {
template <class H, class Tpl> class call_helper;
template <class H, class... Ts> class call_helper<H, std::tuple<Ts...>> {
public:
  template <u64... Is>
  void operator()(BufferOnHad data, std::index_sequence<Is...>) const noexcept {
    H::recived([&data]() -> Ts {
      return from_bytes<Ts>(data.From(
          tuple_front_i_size<Is, std::tuple<Ts...>>::value, sizeof(Ts)));
    }()...);
  ...
  void operator()(BufferOnHad data) const noexcept {
    operator()(data, std::make_index_sequence<sizeof...(Ts)>());
  };
};
...
struct breaked {
  const ID id;
  const u16 length;
  const BufferOnHad content;
  const BufferOnHad next;
};
std::optional<breaked> get_break(BufferOnHad data);

template <class H> class Parser;
template <template <class P> class H, class P> class Parser<H<P>> {
public:
  Parser() noexcept {
    loop<typename P::tuple>([this]<u64 i, typename T>() {
      parsers[i] = [](BufferOnHad data) {
        if (data.size > P::template size<i>) {
          call_helper<H<P>, typename P::template item<i>::tuple>()(data);
        }
      };
    });
  }
...
private:
  std::array<std::function<void(BufferOnHad)>, P::count> parsers;
};

template <class Protocol> class Serialize;
template <class... Is> class Serialize<idl_protocol::Protocol<Is...>> {
  using P = idl_protocol::Protocol<Is...>;

public:
  Serialize() noexcept {}

  BufferOnHeap operator()(const auto &...var) const {
    using item = idl_protocol::Item<std::remove_cvref_t<decltype(var)>...>;

    BufferOnHeap buffer(sizeof(ID) + sizeof(u16) + item::size);
    to_bytes(buffer, ID(tuple_match_i<item, typename P::tuple>::value),
             (u16)item::size, var...);
...
template <class Handler> inline void parser(BufferOnHad data) {
  static helper::Parser<Handler> parser;

  parser(data);
}

template <class Protocol> inline BufferOnHeap serialize(auto &&...var) {
